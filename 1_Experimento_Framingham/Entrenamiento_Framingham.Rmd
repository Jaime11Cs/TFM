...
1. **Modelo M**: solo incluye las variables (`M`), es decir, todas las variables del dataset excluyendo `male`, `age`, `education` y `BMI`. Se espera que este modelo tenga la menor capacidad predictiva, al no incluir las variables confusoras más relevantes.
...
3. **Modelo M + always.split(C)**: incluye únicamente las variables `M`, pero forzando que cada una de las variables confusoras (`C`) se utilice como variable de división en los árboles de decisión mediante el parámetro `always.split.variables` del paquete `ranger`. Este parámetro permite que determinadas variables estén siempre disponibles como candidatas a dividir en cada nodo. El objetivo es evaluar si forzar el uso de estas variables confusoras permite alcanzar un rendimiento similar (o incluso superior) al modelo MC, reduciendo además la complejidad computacional en escenarios con grandes volúmenes de datos.

Este diseño experimental permitirá evaluar no solo el impacto predictivo de las variables confusoras, sino también el valor práctico del parámetro `always.split.variables` en entornos reales.
...
```{r}
setwd("~/TFM_GitHub/TFM/1_Experimento_Framingham")
```
...
Una vez hecho todo el preprocesamiento de los datos, voy a pasar a entrenar los diferentes modelos.
...
En el código anterior optimizo los hiperparámetros usando un Cross validation de 5. Eligo la mejor combinación de hiperparámetros en base a la Accuracy
...
Eligo la mejor combinación de hiperparámetros en base a la Accuracy
...
```{r}
resultado_boot_MC <- modelo_bootMC$resample
media <- mean(resultado_boot_MC$Accuracy)
ic <- quantile(resultado_boot_MC$Accuracy, probs = c(0.025, 0.975))
...
resample_Msplit <- lapply(names(boot_splitM), function(var_c) {
...
resample_total <- bind_rows(resample_M, resample_MC, resample_Msplit)

ggplot(resample_total, aes(x = Modelo, y = Accuracy, fill = Modelo)) +
  geom_boxplot(color = "darkgreen") +
  theme_minimal() +
  labs(
    title = "Comparativa de Accuracy entre modelos del experimento Framingham",
    x = "Modelo",
    y = "Accuracy"
  ) +
...
## Resultados RF con Dataframe MC sobre Test
...
## Resultados RF con `always.split.variables` sobre Test

```{r}
pred_education <- predict(boot_splitM[["education"]], newdata = dataset_con_split[["education"]]$test)
...
```{r}
pred_age <- predict(boot_splitM[["age"]], newdata = dataset_con_split[["age"]]$test)
...
```{r}
pred_BMI <- predict(boot_splitM[["BMI"]], newdata = dataset_con_split[["BMI"]]$test)
...
```{r}
pred_male <- predict(boot_splitM[["male"]], newdata = dataset_con_split[["male"]]$test)
...
Este modelo, construido únicamente con variables no confusoras, presenta una:

- `Balanced Accuracy` = 0.60  
- `Sensitivity` = 0.65  
- `Specificity` = 0.55  

Muestra un rendimiento limitado: la baja especificidad y la sensibilidad moderada reflejan que el modelo tiene dificultades tanto para identificar individuos en riesgo como para evitar falsos positivos. Esto era esperable al haber excluido variables claves como `age` o `male`.
...
Se entrenaron modelos independientes incluyendo las variables del conjunto M más **una única variable confusora forzada** en los nodos de división mediante el parámetro `always.split.variables`. El objetivo era evaluar si el uso explícito de variables clínicamente relevantes en los splits podía compensar la exclusión del resto de variables clínicas.
...
A partir de los resultados obtenidos en este análisis, se pueden extraer varias conclusiones clave relacionadas con los objetivos planteados. El principal objetivo era evaluar si forzar la división de nodos en variables confusoras consideradas críticas mediante el parámetro `always.split.variables` podía ofrecer un rendimiento comparable o incluso superior al uso del conjunto completo de variables (`MC`).
