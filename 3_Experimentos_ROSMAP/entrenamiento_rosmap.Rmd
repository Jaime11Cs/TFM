...
La demencia se define como el deterioro adquirido de las capacidades cognitivas que dificulta la realización satisfactoria de las actividades de la vida diaria. El Alzheimer es una demencia progresiva que tiene el déficit de memoria como uno de sus síntomas más tempranos y pronunciados. Por lo general, el paciente empeora progresivamente, mostrando problemas perceptivos, de lenguaje y emocionales a medida que la enfermedad avanza [@romano2007enfermedad].

La enfermedad de Alzheimer es la principal causa de demencia entre los adultos mayores. Se trata de una patología de etiología compleja, en algunos casos hereditaria [@donoso2003enfermedad], que se caracteriza clínicamente por un deterioro progresivo de la memoria y otras funciones cognitivas, y neuropatológicamente por la acumulación de placas de β-amiloide, ovillos neurofibrilares de **proteína tau hiperfosforilada**, así como otros procesos como la esclerosis hipocampal o la presencia de cuerpos de Lewy [@bennett2018rosmap].

Estudiar los mecanismos biológicos subyacentes a la demencia tipo Alzheimer y su evolución clínica requiere datos detallados a múltiples niveles: clínico, neuropatológico y transcriptómico. En este contexto, dos estudios longitudinales — el **Religious Orders Study (ROS)**, iniciado en 1994, y el **Rush Memory and Aging Project (MAP)**, iniciado en 1997— y conjuntamente conocidos como *ROSMAP*, constituyen una de las principales fuentes de datos sobre envejecimiento y neurodegeneración. Ambos estudios reclutan personas mayores sin demencia al inicio: ROS inscribe a monjas, sacerdotes y hermanos religiosos de todo Estados Unidos, mientras que MAP incluye personas laicas del noreste de Illinois. Todos los participantes consienten la donación post mortem de tejido cerebral, lo que permite un enfoque clínico-patológico integral [@bennett2018rosmap].

Hasta finales de 2017, los estudios habían inscrito a 3.414 personas, de las cuales el 72,6% eran mujeres y el 88,2% eran personas blancas no latinas. Para estos participantes, el proyecto ROSMAP ha recopilado datos clínicos y cognitivos, junto con perfiles multi-ómicos del cerebro y la sangre (incluyendo transcriptómica, epigenómica, proteómica y metabolómica), además de evaluaciones neuropatológicas detalladas. Esta riqueza de datos ha permitido avances significativos en el estudio de la progresión clínica del Alzheimer, la identificación de fenotipos no cognitivos, y la caracterización de factores de resiliencia cerebral, es decir, la capacidad de algunos individuos para resistir mejor los efectos de la patología cerebral [@bennett2018rosmap].

En este trabajo se utilizarán datos reales del proyecto **ROSMAP** para construir modelos predictivos de la variable `cogdx` [@bennett2018rosmap]. Esta variable representa el diagnóstico clínico final que se asigna en el momento del fallecimiento por médicos del estudio, basado en la historia clínica y una evaluación neuropsicológica detallada, realizada sin conocimiento de los hallazgos neuropatológicos. La escala `cogdx` incluye las siguientes categorías:
...
-   **5. AD con comorbilidad**: Alzheimer con otra causa adicional de deterioro cognitivo [@higginbotham2023unbiased].

Además, se integrarán datos transcriptómicos de expresión génica (matriz `M`) y variables clínicas (matriz `C`). Se aplicarán técnicas de filtrado y reducción de dimensionalidad para obtener una matriz optimizada de predictores génicos (`M'`), y se evaluará el efecto de incluir variables confusoras como el sexo, el RIN (RNA Integrity Number), el intervalo post mortem, y las clasificaciones neuropatológicas de Braak y CERAD.

# Objetivos

El presente trabajo tiene como finalidad explorar el potencial predictivo de datos transcriptómicos y clínicos en relación con el diagnóstico clínico de demencia (`cogdx`) en individuos del estudio **ROSMAP**. Para ello se desarrollarán modelos de clasificación utilizando matrices de expresión génica y covariables clínicas, evaluando el impacto de distintos esquemas de preprocesamiento y control por variables confundidoras.
...
A continuación se contruirán distintos modelos de predicción del estado cognitivo utilizando diversas combinaciones de datos: la matriz de expresión génica completa (M), la matriz filtrada (M'), las variables clínicas (C), y las combinaciones MC y M'C, junto con el parámetro `always,split.variables` para comprobar si forzar la divisón en una variable informativa mejroa el rendimiento del modelo. En todos los casos, se tendrá en cuenta el posible efecto de variables confundidoras conocidas como el sexo (`msex`), el intervalo post-mortem (`pmi`), el número de integridad del ARN (`rin`), el estadio de Braak (`braaksc`) y la clasificación CERAD (`ceradsc`), que se incorporarán explícitamente como variables forzadas en los modelos.

Finalmente, se evaluará el rendimiento de los modelos mediante métricas adecuadas, como la **Accuracy**, comparando el impacto relativo de la información transcriptómica frente a la clínica, así como el efecto de la reducción de dimensionalidad sobre la capacidad predictiva.
...
```{r}
setwd("~/Escritorio/TFM/DatosReales")
```
...
La matriz `C` es un data.frame con 342 indviduos (observaciones) y 13 variables clínicas (columnas). Incluye:
...
La matriz `M` contiene los niveles de expresión génica residualizados para los para los mismos 342 individuos presentes en la matriz `C`. Las filas representan los sujetos y las columnas corresponden a genes o transcritos previamente ajustados por variables técnicas (edad, sexo, RIN, lote experimental).

Cada columna de `M` representa la expresión de un gen específico ya transformada para eliminar la variabilidad atribuible a factores de confusión. Esta matriz constituye la base del análisis transcriptómico, tanto en su forma original (`M`) como tras el filtrado de predictores (`M'`).
...
top20_pairs <- correlated_pairs[order(abs(correlated_pairs$Correlation), decreasing = TRUE), ][1:20, ]

# Obtener los genes únicos implicados en esos 20 pares
genes_top20 <- unique(c(top20_pairs$Variable1, top20_pairs$Variable2))

sub_cor_matrix <- cor_matrix[genes_top20, genes_top20]

corrplot(sub_cor_matrix,
         method = "circle",
         type = "lower",
         tl.col = "black",
         tl.srt = 45,
         ...
         title = "Top 20 genes altamente correlacionados",
         mar = c(0, 0, 2, 0))
```
...
Como paso previo al filtrado de varianza, se evaluó la correlación entre los genes de la matriz de expresión `M`. Se detectaron múltiples pares de predictores con correlaciones absolutas sueriores a 0.8, y se eliminaron aquellos predictores mediante el algoritmo `findcorrelation` con un umbral de 0.9.
...
Las primeras variables eliminadas fueron:

```{r}
head(genes_eliminados, 10)
```
...
Ahora que ya se han eliminado los genes redundantes se va a pasar a construir las combinaciones de predictores:
...
-   **C** es la matriz de covariables clínicas, aunque de estas solo se utilizarán `msex`, `pmi`, `rin`, `braaksc`, `ceradsc` y `batch` y la variable a predecir `cogdx`
...
cofounders <- C %>% select(all_of(vars_C))
...
-   **C:** conjunto de covariables clínicas
-   **MC:** combinación de M con C.
-   **M'C:** combinación de M' con C
-   **M + always.split.variables:** modelo de M forzando las variables clínicas en los splits
-   **M' + always.split.variables:** modelo de M' forzando las variables confundidoras en los splits.

El flujo de trabajo para cada modelo consistirá en:

1.  Optimización de hiperparámetros mediante validación cruzada, empleando la métrica de **Accuracy**
2.  Entrenamiento final mediante **bootstrapping (100 repeticiones)** con los hiperparámetros óptimos.
3.  Cálculo de la media y el **intervalo de confianza del 95% de Accuracy**
4.  Obtención de la **importancia de variables**
5.  Entrenamiento adicional forzando las variables confundidoras en los splits (`always.split.variables`) cuando corresponda.
...
Se observa un claro desbalance entre las 5 clases a predecir. Siendo la categoría 4 (Enfermedad de Alzheimer sin otras causas adicionales) la más frecuente, seguido de la categoría 1 (Sin deterioro cognitivo). Este desbalance refuerza la necesidad de emplear métricas robustas como la **Balaced Accuracy** durante el proceso de validación y evaluación de los modelos. También implica que antes de entrenar los modelos y cuando hayamos generado los datos de train estos se van a tener que balancear de alguna manera.
...
Ahora para que el entrenamiento del modelo se haga de manera óptima es necesario balancear los datos. Para este paso voy a usar `upSample` de `Caret`, esta función hace un muestreo aleatorio con reemplazo de la clase minoritaria hasta que tenga el mismo tamaño que la clase mayoritaria [@kuhn2008building].
...
```{r}
modelo_cv_M <- readRDS("ModeloM/modelo_cv_M.rds")
```
...
```{r}
modelo_bootM <- readRDS("ModeloM/modelo_bootM.rds")
```
...
```{r}
varimp_listM <- readRDS("ModeloM/varimp_list_M_cvboot_15rep.rds")
```
...
```{r}
ranking_matrixM <- readRDS("ModeloM/ranking_matrix_M_cvboot_15rep.rds")
```

Una vez cargada, calculo la **Correlación de Spearman** entre los rankings de cada par de repeticiones. Esto es fundamental para evaluar la consistencia del Ranking. Si los modelos son estables, se espera que las repeticiones estén altamente correlacionadas entre sí.
...
```{r}
gene_variancesM <- apply(ranking_matrixM, 1, var)
...
ggplot(df_stability, aes(x = MeanRank, y = RankVariance)) +
  ...
  labs(
    title = "Estabilidad de los genes en el modelo M",
...
El gráfico anterior muestra la estabilidad de los genes en el modelo M. Los genes **consistentemente importantes** (1-1000) tienden a tener baja varianza si son realmente informativos. Los genes **irrelevantes** (rankink promedio alto, \>7000) tambień suelen tener baja varianza, porque siempre salen al final. Los genes con **importancia inestables** (4000-7000), tienden a tener **alta varianza**, porque en algunas repeticiones aparecen como importantes y en otras no.

Ahora voy a extraer los 20 **genes más estables** (baja varianza), y **más importantes** (ranking promedio más bajo).

```{r}
ranking_esabilidad_ordenado <- df_stability[order(df_stability$RankVariance, df_stability$MeanRank), ]
top20_estables <- head(ranking_esabilidad_ordenado, 20)

ggplot(top20_estables, aes(x = reorder(Gene, -MeanRank), y = MeanRank)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(
    title = "Top 20 genes más estables y con mejor ranking",
    x = "Gen",
    y = "Ranking promedio"
  ) +
  theme_minimal()
...
-   **TRIM5** es una proteína que actúa como facotr de restricción antiviral, bloqueando la replicación de retrovirus en etapas tempranas del cliclo de vida viral. Además de su papel antiviral, **TRIM5** participa en la activicación de respuestas inmunitarias innatas [@zabetian2015retinol].
...
```{r}
modelo_cv_Mprima <- readRDS("Modelo M'/modelo_cv_Mprima.rds")
```
...
```{r}
modelo_bootMprima <- readRDS("Modelo M'/modelo_bootMprima.rds")
```
...
El modelo entrenado sobre la matriz de expresión génica filtrada (M') mostró una **Accuracy** promedio del 83.9%, con un intervalo de confianza del 95% entre 78.8% y 88.5%. Este resultado es ligeramente superior al obtenido con la matriz completa `M`, indicando que el proceso de filtrado por baja varianza y alta correlación no solo no perjudicó el rendimiento del modelo, sino que contribuyó a aumentar su eficiencia.

Este resultado refuerza la importancia de aplicar técnicas de selección de variables en datasets de expresión génica de alta dimensión, facilitando modelos más robustos y potencialmente más interpretables.
...
-   **RBP4** es la principal proteína transportadora de retinol en el plasma, facilitando su movilización desde el hígado hacia los téjidos periféricos. Niveles elevados de **RBP4** se han asociado con resistencia a la insulina, obesidad y diabetes tipo 2 [@nankam2021retinol].
...
```{r}
modelo_cv_MC <- readRDS("Modelo MC/modelo_cv_MC.rds")
```
...
```{r}
modelo_bootMC <- readRDS("Modelo MC/modelo_bootMC.rds")
```
...
```{r}
modelo_cv_MprimaC <- readRDS("ModeloMprimaC/modelo_cv_MprimaC.rds")
```
...
```{r}
modelo_bootMprimaC <- readRDS("ModeloMprimaC/modelo_bootMprimaC.rds")
```
...
La integracción de datos transcriptómicos filtrados junto con a las variables clínicas robustas ofrece un modelo predictivo más eficiente, robusto y potencialmente más aplicable en contextos clínicos. Estos hallazgos refuerzan la importancia de la depuración de los datos en casos donde la complejidad de los datos es elevada.
...
En modelos de Machine Learning aplicados a datos ómicos, la presencia de variables clínicas conocidas por su fuerte asociación con el fenotipo de interés, como las escalas neuropatológicas **ceradsc o braaksc** o covariables como **sex, pmi y rin** pueden actuar como variables **confusoras**. Estas variables, al estar altamente correlacionadas con la clase, pueden dominar el modelo e impedir que se valore adecuadamente la capacidad discriminativa de los genes en este caso.
...
-   Evaluar la capacidad predictiva del conjunto de genes una vez controlado explícitamente por las variables clínicas.
...
```{r}
modelo_cv_Msplit <- readRDS("Msplit/modelo_cv_splitM.rds")
```

Imprimo los mejores hiperparámetros para cada una de las variables clínicas. Estos hiperparámetros se utilizarán individualmente en cada variable clínica del modelo entrenado con **Boostrapping = 100**
...
```{r}
boot_splitM <- readRDS("Msplit/modelo_boot_splitM.rds")
```
...
El entrenamiento con **bootstrapping** confirmó la superioridad de las variables neuropatológicas `braaksc` y `ceradsc` como principales cofounders en la clasificación de Alzheimer, alcanzando unos resultados de **Accuracy** de 95% y 95.9% respectivamente, con unos intervalos de confianzas tamvién estrehos y muy elevados.Por su parte las variables técnicas como `rin`, `batch` o `pmi`, y demográficas como `msex`, mostraron un impacto moderado pero consistente, con unos resultados de **Accuracy** en torno al 86-89%.

Estos resultados confirman que forzar al modelo utilizar variables muy informativas es muy útil ya que permite lograr unos resultados prácticamente idénticos al del modelo completo, reduciendo la complejidad computacional y haciendo los resultados mucho más interpretables. Esto es útil sobre todo en el ámbito médico, donde prácticamente siempre se conocen variables clínicas muy importantes a la hora de predeccir y clasificar una determinada enfermedad, en este caso el Alzheimer.
...
Estos resultados reflejan la diferente magnitud del impacto de cada variable clínica y justifica la necesidad de un control adecuado en modelos transcriptómicos.

## Modelo RF con `M'` + `always.split.variables`.

En este experimento voy a seguir el mismo enfoque que en el anterior, voy a pasar las 6 variables clínicas al entrenamiento de los modelos gracias al parámertro `always.split.variables`, pero en este caso el entrenamiento se hace sobre el los datos transcriptómicos filtrados `M'`. El objetivo es observar si el filtrado de los genes correlacionados permite que el modelo se centre más en la variables clínicas importantes para predecir la enfermedad y por tanto mejoren los resultados.
...
```{r}
modelo_cv_Mprimasplit <- readRDS("Mprimasplit/modelo_cv_splitMprima.rds")
```
...
```{r}
boot_splitMprima <- readRDS("Mprimasplit/modelo_boot_splitMprima.rds")
```
...
La consistencia de los resultados de las variables clínicas confirma la robustez del enfoque, destacando de nuevo a las variables `braaksc` y `ceradsc` como prinicpales factores explicativos en la clasificación de la variable objetivo `Alzheimer`.
...
En este experimento se va a entrenar un modelo, pero solo con la variables clínicas. El objetivo es ver si la información de estas variables es tan alta que se pueda construir un modelo predictivo fiables solo basándose en ellas, o si por el contrario se necesitan los datos transcriptómicos más estas variables.
...
El objetivo de este análisis es evaluar como **influye la introducción de cada variable clínica o técnica** de forma individual en la estabilidad de los rankings de importancia de los genes dentro del modelo de clasificación. Se parte de la hipótesis de que si una variable `c` está relacionada con ciertos genes, al incluirla la posición de esos genes en el ranking de importancia debería estabilizarse. Es decir, su varianza en el ranking de importancia entre repeticiones debería **disminuir** respecto al modelo `M`.
...
Este experimento permite **comparar la varianza del ranking de cada gen** en el modelo M, con la varianza de ese mismo gen en el modelo `M + c`. El cociente entre ambas varianzas indica el grado de influencia de la variable `c` sobre ese gen. Si es menor que 1, el **gen se ha estabilizado** tras introducir `c`, lo que podría indicar que está asociado con esa variables `c`.
...
```{r}
ranking_Mc_list <- readRDS("Modelo Mc (Importancia)/ranking_total_list_Mplus_15rep.rds")
...
donde $r_i$ es la posición del gen $i$ en el ranking de importancia (`varImp`) a lo largo de las 15 repeticiones. Un valor $\text{Ratio}_i < 1$ indica que la inclusión de la covariable $c$ reduce la variabilidad del ranking del gen $i$, lo que puede interpretarse como una estabilización relacionada con dicha variable.
...
A continuación se visualiza, la **relación entre la varianza del ranking de cada gen en el modelo M y en el modelo M + c**. Los puntos por debajo de la diagonal corresponden a genes que **se estabilizan** al introducir la variable, lo que puede indicar **posible relación con ella**
...
ggplot(df_var_ratios, aes(x = Var_M, y = Var_MC)) +
  ...
  labs(
    title = "Comparación de varianzas del ranking de genes",
    subtitle = "Modelo M vs Modelo M + c",
    x = "Varianza en modelo M",
    y = "Varianza en modelo M + covariable"
  ) +
  theme_minimal()
```
...
Los genes que están por debajo de la diagonal tienen **menor varianza** al introducir la covariable, se estabilizan, por lo que pude haber relación con esa covariable. Los genes por encima de la diagonal se vuelven **mas inestables** porque no están relacionados o porque la inclusión de c los altera.

Para facilitar la interpretación, voy a filtrar únicamente los **genes que muestran un cociente de varianza** superior a 1, es decir, aquellos que **se estabilizan al introducir la covariables**. Esto permite visualizar con mayor claridad el subconjunto de genes potencialmente asociados con cada covariables.
...
La tabla muestra los 100 genes con menor ratio para cada una de las covariables. Cuánto más bajo sea el ratio, más se ha estabilizado ese determinado gen al introducir `c`.
...
ggplot(df_top_estables, aes(x = Var_M, y = Var_MC)) +
  ...
  labs(
    title = "Genes estabilizados tras introducir covariables",
    subtitle = "Top 100 genes con Ratio de varianza < 1 por cada covariable",
    x = "Varianza en modelo M",
    y = "Varianza en modelo M + covariable"
  ) +
...
En la figura se muestran, para cada covariable, los 100 genes con menor cociente de varianza (Ratio < 1). La mayoría de los genes se sitúan por debajo de la diagonal, indicando una estabilización de su posición en el ranking de importancia al introducir la covariable correspondiente. 

Las variables clínicas `braaksc` y `ceradsc`producen la mayor estabilización de genes, lo que sugiere que están estrechamente asociadas con la relevancia predictiva de estos genes en la clasificación del estado cognitivo. Por el contrario, variables como `msex`, `pmi` o `rin` presentan un efecto má debil, aunque también presentan un efecto estabilizador sobre algunos genes.

Este análisis apoya la utilidad de `braaksc` y `ceradsc` como covariables clínicas clave y justifica su incorporación en los modelos.

Además de indentificar los genes más estabilizados por cada covariable, también voy a seleccionar los **genes con un ratio menor a 0.001**, manteniendo el registro de en que modelo (M + c) se observó dicha estabilización. Permite explorar si hay covariables que estabilizan más genes que otras.
...
A difrencia de la tabla anterior, esta muestra los genes con un ratio menor a 0.001. Cuanto menor sea el ratio más se estabilizan los genes.

```{r}
ggplot(df_extremos, aes(x = Gene, y = Ratio, fill = Covariable)) +
  geom_col() +
  labs(
    title = "Genes más estabilizados (Ratio < 0.001)",
    subtitle = "Barras apiladas por covariable",
    x = "Gen",
...
En la figura se representan los genes con cociente de varianzas inferior a 0.001, es decir, aquellos cuya estabilidad en el ranking de importancia mejoró de forma drástica al introducir alguna covariable. Cada barra representa un gen, y los colores indican las covariables responsables de la estabilización.

Loa genes `APOBEC3G`, `ARIH2`, `MSL1`, `PDCD10`, `SMYD4`, `TSSC4` y `ZRSR2` se extabilizan únicamente al introducir las variables clínicas `braaksc` y `ceradsc`, lo que sugiere una posible relación con el estado neuropatológico. En cambio, genes como `SPON2` o `ARHGEF11` presentan una estabilización combinada por múltiples covariables, incluidas variables técnicas, lo que apunta a su sensibilidad al ruido experimental y la necesidad de interpretarlos con cautela.

Esta visualización aporta información valiosa para priorizar genes estables y relevantes, descartando aquellos que podrían deber su importancia a factores de confusión. 
...
En modelo `C` ofrece un rendimiento intermedio pero elevado, evidenciando que la potencia explicativa de las variables clínicas por si solas es bastante alta. En cambio, los modelos basados únicamente en expresión génica (`M` y `M'`) obtienen los resultados de Accuracy más bajos, aunque siguen siendo buenos resultados.

La inclusión forzada de covariables mediante el parámetro `always.split.variables` demuestra que controlar factores neuropatológicos como `braaksc` o `ceradsc` mejora sustancialmente la clasificación, mientras que factores técnicos o demográficos como `pmi`,`rin` y `msex` aportan menos valor discriminativo, siendo los resultados prácticamente los mismos que en los modelos basados solo en expresión génica.
...
precision <- conf_M$byClass[, "Pos Pred Value"]
recall <- conf_M$byClass[, "Sensitivity"]
...
print(conf_MC)
```
Este modelo incorpora tanto la matriz de expresión génica `M` como las variables clínicas y técnicas `C`, por lo que refelja el impacto de integrar datos fenotípicos y técnicos junto con datos moleculares.
...
El modelo **MC** demuestra el valor de incluir la matriz de variables para la mejora del rendimiento del modelo, especialmente a la hora de detectar la clase `AD`, donde alcanza una sensibilidad perfecta y un excelente balance.
...
print(conf_MprimaC)
```
Este modelo utiliza la matriz de expresión génica filtrada `M'` junto con las variables clínicas y técnicas `C`. Es un modelo en el que se elimina el ruido de los genes redundantes para comprobar si enmascaraban información importante.
...
El **modelo M'C** ofrece un **rendimiento superior a todos los modelos anteriores** gracias a combinar una matriz de expresión filtrada más la variables clínicas y técnicas. La expresión de genes redundantes no solo no empeora el rendimiento si no que lo mejora.
...
print(conf_C)
```
Este modelo utiliza únicamente las variables clínicas y técnicas `C`, sin incluir ningun datos molecular. El resultado es bastante competitivo, lo que indica que estas variables contienen información relevante para la clasificación. 
