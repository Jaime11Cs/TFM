...
```{r}
setwd("~/Escritorio/TFM/generando_dataset")
```
...
El presente análisis tiene como objetivo evaluar el impacto de variables clínicas potencialmente confusoras sobre la capacidad predictiva de modelos Random Forest en el contexto de una enfermedad simulada (`Alzheimer`). Para ello, se ha simulado un conjunto de datos con estructura casual controladam en el que ciertas variables clínicas (`C`) influyen sobre un subcojunto de genes (`M`), y este a su vez sobre la variable objetivo binaria (`Y`). Además, se han generado genes ruidosos sin relación casual con `C` ni con `Y`, reproduciendo un escenario realista de datos ómicos.
...
- Simular un conjunto de datos realista con variables clínicas (`C`), genes (`M`) y una variable objetivo (`Y`), controlando sus relaciones causales.
...
- Evaluar el redimiento de modelos Random Forest entrenados con diferentes subconjuntos de variables:
  ...
  - `ABD` + variables clínicas
  - Modelos con variables clínicas forzadas en los splits (always.split.variables)
...
```{r}
mydata <- read.csv("datos_simulados.csv")
str(mydata)
```
...
En conclusión, el modelo entrenado sobre este conjunto de datos logra una muy buena capacidad de discriminación entre clases, a pesar de que a priori al no tener las variables clínicas `C` debería ser el peor de los modelos. Voy a pasar a entrenar los otros modelos, ya sí con las variables clínicas y a comparar los resultados, para dilucidar si las variables clínicas ayudan a la eficacia del modelo.
...
El gráfico anterior nos permite ver claramente cuales son los genes más influyentes en el modelo `M`, que **no incluye las variables clínicas**.
...
En la tabla enterior se muestran los mejores hiperparámetros para cada variable clínicas `C`. Ahora entrenamos con **Botstrapping** con los mejores parámetros seleccionados para cada vairbale clínica.
...
- La Balanced Accuracy es alto, y comparable a la del modelo `M`, aunque lejana a la del modelo `MC` que usaba todas las variables
...
- Se observa una **mejora clara** respecto al modelo AB (0.844 → 0.945), lo que sugiere que las variables clínicas **aportan información adicional relevante** para la predición.
...
- Este patrón indica que el modelo **detecta que las variables clínicas son muy predictivas de la variable Y**, incluso más que los propios genes que la generaron (**grupo A y B**).
...
- Los genes del **grupo B** siguen dominando frente a los del **grupo A**, lo que corroborá su mayor importancia. Además si nos fijamos en los genes más importantes después de las variables clínicas, aparecen el **B_gene21** y el **B_gene176**, que ya aparecían como los dos genes mas importantes en el modelo sin variables clínicas, lo que refleja que aunque ahora el modelo se fija mucho más en las variables clínicas importantes (`Edad` y `Sexo`), los mismos genes siguen teniendo importancia en los procesos de aprendizaje del modelo.

Las variables clínicas `C` dominan el modelo cuando se incluyen, lo que refleja tanto su impacto directo sobre `Y`como su rol como cofounders que afectan a `M`. Esto **refuerza la idea de que el modelo puede confundirse** si no se controla bien el efecto de las variables clínicas.
...
Este modelo combina **la información relevante de los genes A y B** + un grupo de genes que no es informativo y que puede llegar a confunidir al modelo + **la información contextual de las variables clínicas**. El resultado es muy bueno

- Se observa una clara mejora frente al modelo `ABD`(0.84 → 0.931). Esto es lógico y sigue la pauta de todos los otros modelos, cuando se añaden las variables clínicas, estas **aportan información adicional relevante** para la predicción y mejoran los resultados.
...
- Pero aunque el resultado es muy bueno, no llega a superar al del modelo **AB + C**, es decir el modelo que solo tiene genes informativos + las variables clínicas. 
...
-Si comparamos este resultado con el del modelo **AB_C** nos damos cuenta que los valores de importancia de las variables clínicas en este modelo han disminuido, mientras que la importancia de los genes ha aumentado. Esto puede ser la clave del porque los resultados del modelo han disminuido. Mientras que el modelo anterior se fia mucho más en las variables clínicas, mucho más importantes, este modelo, quizá debido a la introducción del **grupo de genes D** dsiminuye un poco la importancia de las variables clínicas (Mucho más  importantes que los genes) para darle más valor a los genes.
...
Ahora voy a entrenar un modelo solo con las variables clínicas. Este experimento permite comprobar cuánto poder predictivo tienen solo las variables clínicas. Nos ayudará a interpretar si, por ejemplo, edad y sexo que son las variables mas importantes, por sí solas pueden predecir la variable objetivo `Y` con precisión. Este enfoque será clave para comparar con otros modelos.
...
El modelo consigue una Balanced Accuracy del 82.9%, lo que es representa un buen resultado, sobre todo teneindo en cuanta que no dispone de información genética (`M`), sino únicamente de las **4 variables clínicas**. 
...
Este resultado resalta la hipótesis causal planteada, donde las variables clínicas no solo incluyen en ciertos genes, sino que también tienen un impacto muy fuerte sobre `Y`. 
...
resample_M <- modelo_bootM$resample %>%
  rename(BalancedAccuracy = `BalancedAccuracy.Balanced Accuracy`) %>%
  mutate(Modelo = "M")
...
ggplot(resample_total, aes(x = Modelo, y = BalancedAccuracy, fill = Modelo)) +
  ...
  theme_minimal() +
  labs(
    title = "Comparativa de Balanced Accuracy entre modelos",
    x = "Modelo",
    y = "Balanced Accuracy"
  ) +
...
- La mejora al añadir C sugiere que el modelo es capaz de **incorporar variables clínicas relevantes junto a los genes para maximizar rendimiento**
...
-**AB** (Solo genes relevantes) mejora ligeramente, mostrando que la selección de genes útiles ayuda, pero el rendimiento aún es limitado sin las variables clinicas
...
## Modelo únicamente con las variables clínicas
...
-**El mejor modelo es AB + C**, que incluye genes con relación causal y variables clínicas relevantes
